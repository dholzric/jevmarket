...
\paragraph{How much it inflates agreement, measured.} Rather than leave this as
a caveat we bounded it, on a preregistered test that could only hurt the
result. We drew 60 distinct states from the registered population---those
rendered inside the fifteen-period post-shock windows of the confirmatory
\texttt{jev\_argmax} runs, seeds 24--43---and called each five times live,
bypassing the cache. Every raw response is archived.

\begin{center}
\begin{tabular}{lrr}
\toprule
& Estimate & Registered criterion \\
\midrule
F1: states fully stable over five repeats & $57/60 = 0.950$ & \\
\quad Jeffreys one-sided 95\% lower bound & $0.887$ & must exceed $0.95$ \\
\quad \emph{verdict} & \multicolumn{2}{r}{\textbf{does not hold}} \\
\midrule
F2: inflation in agreement from memoisation & $+1.43$pp & \\
\quad one-sided 95\% upper bound & $+2.84$pp & must be under $10$pp \\
\quad \emph{verdict} & \multicolumn{2}{r}{\textbf{holds}} \\
\bottomrule
\end{tabular}
\end{center}

\textbf{F1 does not hold.} The proportion of states whose modal action is
identical across five repeats is $0.950$, but its lower confidence bound is
$0.887$, so at this sample size we cannot certify stability above the
registered threshold. We therefore do not claim the modal policy is effectively
deterministic.

\textbf{F2 holds, and it is the claim that matters.} Memoisation inflates
within-state agreement by at most $2.84$ percentage points with 95\%
confidence, against a measured liquidity effect of $25.8$ points. The mechanism
does not rest on memoisation. What we may assert is the bound, not the stronger
statement that the argmax result transfers unchanged to independent callers.

An earlier version of this analysis reported both criteria as holding. It
sampled the whole response archive rather than the registered post-shock
population, and scored F1 on mean within-state stability rather than the
registered proportion. That deviation, and this correction, are recorded in the
preregistration; the earlier numbers are retained there as exploratory.

Two narrower caveats remain. The sampling arm draws from \emph{one frozen
distribution} per state, and we have not run it under repeated responses.
And F2 bounds instability conditional on the sampled states; it does not
re-run the dynamic market with independent calls throughout.
