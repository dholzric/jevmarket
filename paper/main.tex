...
halt is mild and the up-side halt severe. The bias is \emph{unchanged} under
the mirror wording of Section~\ref{sec:wording}, which eliminates the stated
conviction asymmetry entirely---so the two findings are independent, and the
bias is not an artefact of our phrasing.}
\label{tab:flow}
\end{table}
...
Two explanations are ruled out. It is not the market's construction: both
symmetric baselines are balanced at every market size. And it is not our
question wording: under the mirror phrasing of Section~\ref{sec:wording}, which
