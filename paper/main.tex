...
reused across agents and periods}. Under modal decoding the two coincide more
closely than one might expect---the rounding probe shows the model is decisive
across the whole realistic range of private values, so independent calls would
very often return the same mode anyway---but they are not identical, and
memoisation can only increase within-state agreement. For the sampling arm the
gap is larger: our traders draw independently from \emph{one frozen
distribution} per state, whereas independent callers would draw from
distributions that themselves vary.

The consequence is a scoping one. The argmax result is evidence about memoised
model agents under shared information; generalising it to every deployment of
modal decoding requires a repeated-response condition we have not run. It also
qualifies the sentence ``a deterministic decoder makes them act identically'':
a deterministic decoder returns identical actions only for identical input
distributions, and memoisation is part of what makes the inputs identical here.
A sensitivity analysis drawing several live responses per state is the right
follow-up.
