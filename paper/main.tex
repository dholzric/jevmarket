...
We emphasise the failures. Four preregistered predictions in this project did
not survive, and each is reported: the wording-to-prices hypothesis (C1, a
clean null), the original mechanism (Section~\ref{sec:wording}),
...
establishes \emph{that} the modal arm carries a standing buy bias and that this
bias determines which shock the halt strikes. It does not establish \emph{why}.
...
removed by our question wording: under the mirror phrasing of Section~\ref{sec:wording}, which
removes the stated conviction asymmetry entirely ($+0.005$ against $+0.250$),
the order-flow bias is unchanged---$48.1\%$ buys against $45.7\%$ sells after a
down shock, versus $48.8\%$ against $42.3\%$ under domain wording. Whatever
produces the bias survives making the question symmetric.

What remains is the model itself, the task framing, or the state
representation, and we cannot presently separate these. We flag it as the
...
spend was approximately \$3.85 across roughly 118{,}628 live calls. These figures, and SHA-256 hashes of every result file and archive bundle, are generated from disk into 	exttt{data/manifest.json} rather than transcribed.
