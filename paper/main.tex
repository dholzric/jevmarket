...
signal, so a deterministic decoder makes them act identically, and a market of
...
exploratory and labelled as such. Of seven registered predictions, three were
confirmed (D1, D2, E1), three failed (C1, C2, and the original clause that
modal decoding would be immune) and one is unresolved (E2); all are reported.
A separate post-hoc mechanism account is withdrawn and identified as such.
\end{abstract}
...
\textbf{Three preregistered predictions failed}: C1, that the wording
asymmetry would move pricing error (a clean null, Section~\ref{sec:wording});
the original clause that modal decoding would be immune to the liquidity
effect; and C2, that the sampling arm's wording contrast would be
\emph{smaller} than the argmax arm's, on which the estimate came back the wrong
sign ($+0.161$, $p=0.80$).
\textbf{One is unresolved}: E2 (Section~\ref{sec:size}). \textbf{Three were
confirmed}: D1, D2 and E1.
...
\paragraph{The estimand is memoised, and this narrows the claim.} Requests are
...
reproduce these numbers. Every response is archived---118{,}968 of them---and
the analysis can be replayed against the archive with live calls disabled, in
which case a cache miss raises rather than silently contacting the API.
