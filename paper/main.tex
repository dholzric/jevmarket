...
For a single decision that selects the most probable action, which is the right
choice under 0--1 loss against the returned distribution. This paper
...
effect; and C2, that sampling would show the \emph{larger} wording contrast, on
which the estimate came back the wrong sign ($+0.161$, $p=0.80$).
...
reproduce. All 118{,}628 archived responses are retained and every reported figure is
...
would appear as exactly this bias. We asked the same 50 states under two
arrangements differing only in which label carried which description:
$48/50$ states selected the same economic meaning under both, and the shift in
$P(\text{buy})$ was $+4.0$ points---in the \emph{opposite} direction to a
first-option preference, since moving buy off \texttt{option\_a} made it
slightly more likely. At four points it can account for at most a sixth of the
residual, and the mirror control used in Section~\ref{sec:wording} is not
contaminated by its option naming.
...
result. We drew 50 distinct states the confirmatory runs actually visited and
called each five times live, bypassing the cache.

\begin{center}
\begin{tabular}{lr}
\toprule
Modal action identical across five live repeats & $47/50$ states \\
Mean mode stability & $0.976$ \\
\midrule
Agreement among eight traders, memoised & $1.000$ \\
Agreement among eight traders, each calling independently & $0.979$ \\
\midrule
Inflation attributable to memoisation & $+2.1$pp (SE $1.2$) \\
\bottomrule
\end{tabular}
\end{center}

Both registered criteria held: mode stability at least $0.95$ (F1), and
inflation under ten percentage points (F2). Memoisation raises within-state
agreement by about two percentage points, against a measured liquidity effect
of $25.8$ points. The mechanism does not depend on it, and the argmax result
carries over to independent callers with that bound attached. Had either
criterion failed, the preregistration required rewriting the mechanism section
or reporting the headline number as an upper bound.

The caveat that remains is narrower and applies to the sampling arm: our
traders draw independently from \emph{one frozen distribution} per state,
whereas independent callers would draw from distributions that themselves vary
slightly. The measured variation is small---probabilities move by about
$\pm 0.01$ between repeats---but we have not run the sampling arm under
repeated responses.

\section{Reproducibility}

Because the model is non-deterministic and has no seed, code alone does not
reproduce these numbers. Every response is archived---118{,}628 of them---and
...
The archive holds $118{,}628$ \emph{unique} responses, one per distinct
...
appear separately. Estimated spend from the archived token counts is
