...
unanimity is the cause, the effect must vanish as the market grows. It does,
...
exploratory and labelled as such. Four preregistered predictions did not
survive, and each is reported.
\end{abstract}
...
We emphasise the failures. Four preregistered predictions did not survive, and
each is reported where it occurred: the wording-to-prices hypothesis (C1, a
clean null, Section~\ref{sec:wording}), the claim that modal decoding would be
immune to the effect, the prediction that sampling would \emph{amplify} it, and
the two-component mechanism this draft withdraws (Section~\ref{par:two}). A
fifth registered prediction, E2, is unresolved rather than failed
(Section~\ref{sec:size}). The result that survived did so against a test
designed to kill it.
